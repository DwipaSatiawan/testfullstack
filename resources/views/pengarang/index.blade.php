                        ...
                        <div class="card-box table-responsive">
                            <table id="datatable-all" class="table table-striped table-bordered nowrap" style="width:100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $item->pengarang_id }}</td>
                                        <td>{{ $item->nama }}</td>
                                        <td>
                                            <a href="#" data-toggle="modal" data-target="#crud-data" data-id="{{ $item->pengarang_id }}" class="edit-data btn btn-info btn-sm" title="Ubah Data"><i class="fa fa-pencil"></i> Ubah</a>
                                            <a href="#" data-toggle="modal" data-target="#delete-data" data-id="{{ $item->pengarang_id }}" class="delete-data btn btn-danger btn-sm" title="Hapus Data"><i class="fa fa-trash"></i> Hapus</a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            ...
                            <div class="d-flex justify-content-center">
                                {!! $pengarang->links('pagination::bootstrap-4') !!}
                            </div>
                        </div>
                    </div>
                </div>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title judulmodal" id="myModalLabel">Tambah Pengarang</h4>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on('click', '.edit-data', function(e) {
            e.preventDefault();
            var pengarangId = $(this).attr('data-id');
            $.ajax({
                type: 'GET',
                url: '/pengarang_get_edit/' + pengarangId,
        ...
        $('#form-create').on('submit', function(event) {
            ...
            var pengarangid = $("#crud-data").find("input[name='id']").val();
            var nama = $("#crud-data").find("input[name='nama']").val();

            var formData = new FormData(this);
            formData.append('pengarangid', pengarangid);  // Menambahkan pengarangid
            formData.append('nama', nama);  // Menambahkan nama pengarang

            if (btnform == "Simpan") {
                var urlform = "/pengarang_save";  // URL untuk menyimpan data
                var notifform = "Berhasil simpan data";
            }

            if (btnform == "Ubah") {
                var urlform = "/pengarang_update/";  // URL untuk update data
        ...
        $('#form-delete').on('submit', function(event) {
            ...
            $.ajax({
                url: "/pengarang_delete/" + iddel,  // URL endpoint untuk menghapus
                ...
                success: function(data) {
                    ...
                    sessionStorage.setItem('notifSukses', 'Pengarang berhasil dihapus');  // Menyimpan pesan sukses di sessionStorage
                    window.location.reload();  // Refresh halaman untuk menampilkan data terbaru
                },
                error: function(xhr) {
                    // Jika terjadi kesalahan
                    notiferror('Terjadi kesalahan saat menghapus pengarang');
                }
            });
        });
